...
export const login = async (email: string, password: string): Promise<User | null> => {
  try {
    ...
    // fall back to checking local users database.
    if (!data && !error) {
      console.warn('[Auth] Backend not connected. Attempting local users login.');

      // Check already cached session first
      const cached = getCurrentUser();
      if (cached && cached.email.toLowerCase() === email.toLowerCase()) return cached;

      // Check against local users DB (pos_users_db)
      try {
        const stored = localStorage.getItem('pos_users_db');
        const localUsers: any[] = stored ? JSON.parse(stored) : [];

        // Also include default mock users if DB is empty
        const MOCK_USERS = [
          { id: 'u1', username: 'admin',       fullName: 'System Administrator', role: 'admin',                  branch: 'Main HQ',       email: 'admin@pos.com',       organizationId: 'org-001', password: 'admin123' },
          { id: 'u2', username: 'manager',      fullName: 'Store Manager',        role: 'manager',                branch: 'Main HQ',       email: 'manager@pos.com',     organizationId: 'org-001', password: 'manager123' },
          { id: 'u3', username: 'cashier',      fullName: 'Front Desk Cashier',   role: 'cashier',                branch: 'Main HQ',       email: 'cashier@pos.com',     organizationId: 'org-001', password: 'cashier123' },
          { id: 'u4', username: 'repairman',    fullName: 'Tech Repairman',       role: 'technician',             branch: 'Service Center', email: 'repairman@pos.com',   organizationId: 'org-001', password: 'repair123' },
          { id: 'u5', username: 'commercial1',  fullName: 'Sales Commercial 1',   role: 'commission_commercial',  branch: 'Main HQ',       email: 'commercial1@pos.com', organizationId: 'org-001', password: 'commercial123' },
          { id: 'u6', username: 'commercial2',  fullName: 'Sales Commercial 2',   role: 'commission_commercial2', branch: 'Main HQ',       email: 'commercial2@pos.com', organizationId: 'org-001', password: 'commercial456' },
        ];

        const allUsers = localUsers.length > 0 ? localUsers : MOCK_USERS;
        ...
        if (match) {
          const user: User = {
            ...
            role: match.role,
            branch: match.branch,
            organizationId: match.organizationId || 'org-001',
            permissions: match.permissions || DEFAULT_PERMISSIONS[match.role as keyof typeof DEFAULT_PERMISSIONS] || DEFAULT_PERMISSIONS.customer,
          };
          tokenManager.set('local-jwt-placeholder');
          const persisted = persistUser(user);
          activityLog.logLogin(persisted.id || match.id, persisted.username);
          return persisted;
        }
      } catch (e) {
        console.error('[Auth] Local user lookup failed:', e);
      }
